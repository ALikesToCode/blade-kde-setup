...
- Upstream `bin/cloakserve` SHA-256:
  `a334ec5aaf2221e8a463a7c64cfa9b290e62348582da7e03e24f927b285df1fa`
- Reviewed persistence and headed-mode compatibility patch:
  `extras/hardened-workspace/cloakserve-codex-safe.patch`
- Reviewed upgrade patch for installations using the earlier persistence patch:
  `extras/hardened-workspace/cloakserve-graceful-close-upgrade.patch`
- Installed patched `bin/cloakserve` SHA-256:
  `14e10cb340a0a8e37b60f90742fe01f021035e77c8eb43b9ca98c228a3b455ef`
...
The installer verifies a known upstream or prior patched `cloakserve` hash
before applying the reviewed compatibility patch, then enforces the final hash
at every `codex-safe` launch. The patch keeps the private profile, asks Chromium
to commit it through CDP before shutdown, and removes the presence-sensitive
headed-mode switch before launching Chromium. The standalone ShellCheck hash is
enforced by the installer and doctor. The release archive was verified before
extraction; auto-update is disabled during sandbox sessions so a session cannot
replace the pinned browser.
